...
test('demo session service exposes opaque IDs and rejects arbitrary paths', async () => {
  ...
  assert.ok(['passed', 'failed', 'unknown'].includes(fullRun.diagnosis.executionGate))
  assert.equal(fullRun.diagnosis.businessGate, 'not_evaluated')
  assert.match(fullRun.diagnosis.caveat, /业务验收/)
