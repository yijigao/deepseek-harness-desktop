// Candidate-only r11 builder. It never reads or changes the active release descriptor.
const path = require('node:path')
const fs = require('node:fs')
const { spawnSync } = require('node:child_process')
const root = __dirname
const base = require('./app/package.json').build
const evidence = path.join(root, 'release-artifacts', 'r11-final-source-evidence-20260913')
...
if (baseline.releaseId !== 'desktop-2.2.0-alpha2-r11') throw Error('Unexpected r11 release identity')
