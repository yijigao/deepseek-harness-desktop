// A prepared runtime is a build input, never the old staging/payload tree.
...
const output = path.join(root, 'release-artifacts', require('./maintenance/baseline.json').releaseId)
...
module.exports = {
  ...
  extraResources: [
    ...
    { from: path.join(root, 'maintenance/version.json'), to: 'version.json' },
